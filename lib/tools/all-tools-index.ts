...
//   - app/tool/[toolId]/page.tsx (normalisedTools default export)
//
...
const loveDomain: Domain = {
  ...
  tools: loveTools.map(t => {
    ...
    return {
      ...
      name:             t.name,
      emoji:            t.emoji,
      description:      t.tagline ?? t.hook?.slice(0, 120) ?? t.name,
...
const voiceDomain: Domain = {
  ...
  tools: voiceTools.map(t => ({
    ...
    features:         toFeatures(t),
    price:            t.price,
    duration:         (t as any).sessionDurationMinutes
                        ? `${(t as any).sessionDurationMinutes}-min session`
                        : '/month',
    category:         'voice',
...
// ─────────────────────────────────────────────────────────────
// NORMALISED TOOL SHAPE — used by app/tool/[toolId]/page.tsx
// snake_case to match the Tool interface page.tsx expects
// ─────────────────────────────────────────────────────────────
export interface NormalisedTool {
  ...
  requires_image:      'face' | 'palm' | 'both' | null
...
// Raw lookup — all source constants in one flat array for O(1) access
const _rawAll: any[] = [
  ...(loveTools         as any[]),
  ...(wealthTools       as any[]),
  ...(wellnessTools     as any[]),
  ...(lifePathTools     as any[]),
  ...(omniTools         as any[]),
  ...(sacredScriptTools as any[]),
  ...(timeKeeperTools   as any[]),
  ...(voiceTools        as any[]),
]
const _rawById = new Map<string, any>(_rawAll.map(t => [t.id, t]))

function _toNormalised(t: Tool, raw: any): NormalisedTool {
  return {
    id:                  t.id,
    name:                t.name,
    tagline:             t.shortDescription ?? t.description ?? '',
    hook:                raw?.hook          ?? t.description ?? '',
    emoji:               t.emoji,
    domain:              t.category,
    price:               t.price,
    ...
    is_flagship:         raw?.isFlagship    ?? false,
    is_subscription:     !!t.duration?.includes('month'),
    subscription_period: t.duration?.includes('month') ? 'month' : null,
    requires_image:      (t.requiresImageType || null) as 'face'|'palm'|'both'|null,
    requires_partner:    raw?.requiresPartner  ?? false,
    delivery_minutes:    raw?.deliveryMinutes  ?? 20,
    guidance_type:       raw?.guidanceType     ?? null,
    what_you_get:        t.features            ?? [],
    upsell_id:           raw?.upsell?.id       ?? null,
    upsell_name:         raw?.upsell?.name     ?? null,
    upsell_price:        raw?.upsell?.price    ?? null,
  }
}

/** All tools in snake_case shape for app/tool/[toolId]/page.tsx */
export const normalisedTools: NormalisedTool[] = allTools.map(t =>
  _toNormalised(t, _rawById.get(t.id))
)

/** For generateStaticParams in app/tool/[toolId]/page.tsx */
export const getAllToolIds = () => allTools.map(t => ({ toolId: t.id }))

/** Default export — normalisedTools for page.tsx */
export default normalisedTools
