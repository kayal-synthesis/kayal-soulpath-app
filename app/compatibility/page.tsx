      ...
      {step === 'input' ? (
        <Card>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <input type="text" placeholder="Their Name" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})}
            ...
            <div className="flex gap-2">
              <Button type="submit" loading={loading}>Check Compatibility</Button>
              <ShareButton title="Compatibility Challenge" text="See how compatible we are" url="/compatibility" />
            </div>
          </form>
        </Card>
      ) : (
        <Card className="text-center">
          ...
          <h2 className="text-2xl font-serif mb-2">{session?.user?.name} & {formData.name}</h2>
