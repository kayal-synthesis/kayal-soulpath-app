...
/**
 * app/domain/sacred-script/new/page.tsx
 *
 * Shows only the Sacred Script tools the user has purchased.
 * Has nothing to do with Voice of Prophecy.
 * Routes to /domain/sacred-script/[toolId]
 */
...
const TOOL_DOMAIN: Record<string, string> = {
  ...
  'spiritual-scribe':'spiritual','health-scribe':'health',
  'purpose-scribe':'purpose','relationship-scribe':'love',
  'grief-scribe':'grief','parenting-scribe':'all','business-scribe':'wealth',
}
...
export default function NewChatSessionPage() {
  ...
  const [tools,    setTools]    = useState<typeof sacredScriptTools>([])
  ...
  useEffect(() => {
    ...
    Promise.all(
      sacredScriptTools.map(t => checkSub(user.id, t.id).then(active => ({ t, active })))
    ).then(results => {
  ...
  if (!isLoading && !isAuthenticated) {
    return (
      <div style={centreStyle}>
        ...
        <button onClick={() => router.push('/auth/login')} style={btnStyle('#b8966a')}>
          Sign In
        </button>
      </div>
    )
    ...
    return (
      <div style={{ ...centreStyle, gap:16, textAlign:'center', padding:'40px 24px' }}>
        ...
        <p style={{
          ...
          No Sacred Script tools
        </p>
        <p style={{ ...mutedText, maxWidth:260, lineHeight:1.6 }}>
          You have not subscribed to any scribe tools yet.
        </p>
        <button onClick={() => router.push('/domain/sacred-script')} style={btnStyle('#b8966a')}>
          Browse Scribes
        </button>
      </div>
    )
  ...
  return (
    <div style={{
      minHeight:'100vh',
      background:'radial-gradient(ellipse at 50% 0%, #0f0c07 0%, #0d0b0f 60%)',
      ...
      <div style={{ padding:'24px 20px 16px', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12, marginTop:12 }}>
          <div style={orbStyle('#b8966a', 42)}>📜</div>
          <div>
            <h1 style={pageTitle}>Sacred Script</h1>
            <p style={{ fontSize:11, color:'rgba(184,150,106,0.6)', fontFamily:"'EB Garamond',Georgia,serif" }}>
              Your active scribes
            </p>
          </div>
        </div>
      ...
      <div style={{ padding:'16px 16px 0' }}>
        {tools.map(tool => {
          const dc = DOMAIN_ACCENT[TOOL_DOMAIN[tool.id] ?? 'all'] ?? '#b8966a'
          return (
            <ToolRow
              key={tool.id}
              tool={tool}
              accent={dc}
              onClick={() => router.push(`/domain/sacred-script/${tool.id}`)}
            />
          )
        })}
      </div>
    ...
    >
      ...
      <div style={{ flex:1, minWidth:0 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10, marginTop:4 }}>
          {tool.limits?.messagesPerMonth && (
            <span style={{ fontSize:9, color:'rgba(106,96,86,0.55)' }}>
              {tool.limits.messagesPerMonth} msg/mo
            </span>
          )}
          {tool.rating && (
