...
const PARTNER_TOOL_IDS = new Set([
  ...
  'professional-compatibility-scan', 'child-blueprint-os',
  'parent-child-mirror', 'family-blueprint',
])
const needsPartner = (tool: any): boolean =>
  !!(tool.requiresPartner) || !!(tool.requires_partner) || PARTNER_TOOL_IDS.has(tool.id)
...
export default function ToolSalesPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/affiliate/track-click', {
      ...
      body: JSON.stringify({ ref: refCode, tool_id: toolId }),
    }).catch(() => {})
  }, [refCode, toolId])
  ...
  const ctaLabel     = isPartner ? 'Begin Our Reading'  : 'Begin My Reading'
  const ctaLabelFull = isPartner
    ...
    <div className={styles.page}>
          ...
          {isPartner && (
            ...
            >
              ...
              <p style={{ fontSize: '0.85rem', color: cfg.color, margin: 0 }}>
                This reading synthesises two complete blueprints. You will enter both sets of details below.
              </p>
            </motion.div>
          )}
                ...
                <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={styles.teaserForm}>
                  ...
                  {isPartner && (
                    <div style={{ marginTop: '1rem' }}>
                      ...
                      <div className={styles.formGroup}>
                        <label className={styles.formLabel}>Partner full legal name</label>
                        <FocusInput type="text" value={partnerName} onChange={e => setPartnerName(e.target.value)} placeholder="Partner's name as on birth certificate" accentColor={cfg.color} icon={Users} />
                      </div>
                      <div className={styles.formGroup}>
                  ...
                  <p style={{ fontSize: '0.7rem', color: '#a8a29e', textAlign: 'center', marginTop: '0.75rem', marginBottom: 0 }}>
              ...
              {teaserShown && (
                <motion.div key="result" ref={teaserResultRef} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={styles.teaserOutput}>
                  ...
                  <div className={styles.teaserOutputCta}>
                    <p className={styles.teaserCutoff}>Your full reading goes significantly deeper than this preview.</p>
                    <motion.button
                      whileHover={{ opacity: 0.88 }} whileTap={{ scale: 0.97 }}
      ...
      <div className={styles.container} style={{ padding: '0 1.5rem 3rem' }}>
        <motion.div
          ...
          style={{ position: 'relative', aspectRatio: '16/7', borderRadius: 18, overflow: 'hidden', background: atmos.accent }}
        >
          <Image src={`/images/tools/${tool.id}.webp`} alt={tool.name} fill className="object-cover"
            onError={e => { (e.currentTarget as HTMLImageElement).style.display = 'none' }} />
          <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(135deg, ${cfg.color}40, transparent 60%)` }} />
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <span style={{ fontSize: 96, opacity: 0.07 }}>{tool.emoji || '🔮'}</span>
          </div>
        </motion.div>
      </div>
      ...
      <section className={styles.trustSection} style={{ ...SPB, background: atmos.accent, borderTop: 'none', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className={styles.container}>
          ...
          <div style={{ maxWidth: 660, margin: '0 auto 3rem', textAlign: 'center' }}>
            <motion.div
              initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.5, ease: E }}
              style={{ width: 88, height: 88, borderRadius: '50%', margin: '0 auto 1rem', border: '2px solid #e8e3dc', overflow: 'hidden', position: 'relative' }}
            >
              <Image src="/images/creator/founder.webp" alt="Victor Hayford Samson" width={88} height={88} className="object-cover w-full h-full"
                onError={e => { (e.currentTarget as HTMLImageElement).style.display = 'none' }} />
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: cfg.light }}>
                <User style={{ width: 34, height: 34, color: cfg.color }} />
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }}
              ...
              {[
                'For years I kept encountering the same wall. I had access to practitioners of ancient reading traditions that most people in the West have never encountered, and each one offered something genuinely precise. But every reading left me holding a piece of a picture that no one could complete. The disciplines were not in disagreement. They were each describing the same thing from a different angle, and no one was showing the whole shape. That gap is what eventually became KAYAL.',
                'This platform exists for the person who already knows something specific is true about their situation but cannot yet see the full picture of it. Every reading built into KAYAL is drawn from the intersection of multiple disciplines, because that intersection is where the real answers live.',
              ].map((p, idx) => (
                <p key={idx} style={{ fontSize: '0.9rem', color: '#44403c', lineHeight: 1.75, marginBottom: '0.875rem', textAlign: 'left' }}>{p}</p>
              ))}
            </motion.div>
          </div>
